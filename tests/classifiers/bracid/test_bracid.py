...
class TestBracid(TestCase):
    ...
    def test_bracid_stops(self):
        ...
        correct_rules = {
            0: pd.Series({"B": Bounds(lower=1, upper=1), "C": Bounds(lower=2, upper=3.0), "Class": _0}, name=0),
            2: pd.Series({"B": Bounds(lower=0.75, upper=4.0), "C": Bounds(lower=0.5, upper=2.5), "Class": _1}, name=2),
            3: pd.Series({"B": Bounds(lower=0.75, upper=1.5), "C": Bounds(lower=0.5, upper=2.5), "Class": _1}, name=3),
            4: pd.Series({"B": Bounds(lower=0.5, upper=0.75), "C": Bounds(lower=2.5, upper=3.0), "Class": _1}, name=4),
            5: pd.Series({"B": Bounds(lower=0.75, upper=0.875), "C": Bounds(lower=2.0, upper=2.5), "Class": _1}, name=5),
