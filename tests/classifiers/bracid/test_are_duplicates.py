...
class TestAreDuplicates(TestCase):
    ...
    def test_are_duplicates_bounds(self):
        ...
        rules = [
            ...
            pd.Series({"B": Bounds(lower=0.8, upper=1), "C": Bounds(lower=1, upper=1), "Class": _0}, name=2),
        ]
    ...
    def test_are_duplicates_true(self):
        ...
        rules = [
            pd.Series({"B": Bounds(lower=1, upper=1), "C": Bounds(lower=1, upper=1), "Class": _0}, name=1),
            pd.Series({"B": Bounds(lower=1, upper=1), "C": Bounds(lower=1, upper=1), "Class": _0}, name=2),
        ]
        duplicate = _are_duplicates(rules[0], rules[1])
