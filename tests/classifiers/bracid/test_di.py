...
class TestDi(TestCase):
    ...
    def test_di_nan_row(self):
        ...
        correct = [
            pd.Series([1 / 4 * 1 / 4, 0.0, 0.0, 1.0, 1 / 8 * 1 / 8, 0.0], name="A"),
    ...
    def test_di_nan_rule(self):
        ...
        df = pd.DataFrame(
            {"A": ["high", np.nan, "high", "low", "low", "high"], "B": [3, 2, 1, np.nan, 1, 2], "Class": [_0, _0, _1, _1, _1, _1]}
        )
        class_col_name = "Class"
    ...
    def test_di_single_feature(self):
        ...
        df = pd.DataFrame(
            {"A": ["high", np.nan, "high", "low", "low", "high"], "B": [3, 2, 1, 0.5, 1, 2], "Class": [_0, _0, _1, _1, _1, _1]}
        )
        class_col_name = "Class"
    ...
    def test_di_multiple_features(self):
        ...
        df = pd.DataFrame({"A": [1, 1, 4, 1.5, 0.5, 0.75], "B": [3, 2, 1, 0.5, 3, 2], "Class": [_0, _0, _1, _1, _1, _1]})
        class_col_name = "Class"
    ...
    def test_di_multiple_features_multiple_rules(self):
        ...
        class_col_name = "Class"
        rules = [pd.Series({"A": (1, 2), "B": (1, 2), "Class": _1}), pd.Series({"A": (1, 2), "B": (1, 2), "Class": _1})]
        dists = []
        min_max = pd.DataFrame({"A": {"min": 1, "max": 5}, "B": {"min": 1, "max": 11}})
        # Only keep rows with the same class label as the rule
        df = df.loc[df[class_col_name] == _1]
