...
from multi_imbalance.classifiers.bracid.bracid import most_specific_generalization
...
class TestMostSpecificGeneralization(TestCase):
    ...
    def test_most_specific_generalization_multiple_rules(self):
        ...
        dataset = pd.DataFrame(
            {"A": [upper_bound, lower_bound], "B": [lower_bound, upper_bound], "C": [lower_bound, upper_bound], "Class": [_0, _0]}
        )
        class_col_name = "Class"
        rules = [
        ...
        correct = [
            pd.Series({"A": (0.1, upper_bound), "B": (lower_bound, 1), "C": (lower_bound, 2), "Class": _0}),
